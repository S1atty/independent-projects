---
title: "Predicting Patient Satisfaction for Emergency Department Encounters"
...
for (i in 1:ncol(mydata)){
  if (is.character(mydata[,i]) && i != 5 && i != 6){
...
After cleaning records and additional formatting, observations were then cleaned. Survey ID 1418129390 had missing admit and discharge times and the encounter record for the survey was dated 1900-01-01. The implied erroneous record was removed.
...
```{r, eval = TRUE, echo = FALSE, include = TRUE, message=FALSE, fig.width = 15, fig.height=10}
...
gridExtra::grid.arrange(d1,d2,d3, newpage = TRUE)
...
**Wald Statistic**: {\large $β^2/Var(β)$} measures the effect size of individual parameters.**Variance Inflation Factors** greater than four were reevaluated; greater than seven are removed.
...
train %<>% mutate(C75_7_rt = (C75_rt+C7_rt)/2)
...
where β~1~ describes change in \hat{y_1} for a one unit change in the root mean satisfaction for the personal issues category, β~2~ for a one unit change in the root mean satisfaction for the nurses category, and β~3~ for a one unit change in the root indicator for Doctor's concern for patients comfort while treating them. Interpretations for β~0~ is omitted due to transformations and range of the data set. The Adjusted R^2^ \~ .74, F(3,2045), p = 2.2e10 using adjusted Type I error rate = .005.
