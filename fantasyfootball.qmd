---
title: "Principal Component Analysis to Inform Draft Selection for Fantasy Football Teams"
...
fviz_pca_biplot(s,
  ...
  annotate("text", label = "Best Distance Players", x = -12, y = -0.5, color = "grey30") +
  geom_rect(aes(xmin = 0, xmax = 5, ymax = -7.5, ymin = -3), 
            color = "grey30", 
            alpha = 0, 
            show.legend = FALSE) +  # Hide legend for this rect
  annotate("text", label = "High-Volume QBs", x = 3.5, y = -7.2, color = "grey30") 
...
combine %>%
  ...
  mutate(pc1 = (-0.96567341*weighted_PassingYards)+(-0.94166132*weighted_PassingTDs)+
         ...
         pc2 =  (0.962317103*weighted_Receptions)+(0.980608513*weighted_ReceivingTDs)+(0.980608513*weighted_ReceivingYards),
         pc3 = (weighted_RushingYards*0.96620788)+(weighted_RushingTDs*0.93854323),
         ovr = -pc1+pc2+pc3) %>%
  select(Rank2023,PositionRank2023,Team2023,Player2023,FantasyPointsPPR2023,pc1,pc2,pc3,ovr) %>%
  arrange(pc1) %>%
  mutate(ModelRank = row_number(), .before = Rank2023) %>%
  tibble()
...
combine %>%
 # filter(FantasyPosition2023=="RB") %>%
  mutate(pc1 = (-0.96567341*weighted_PassingYards)+(-0.94166132*weighted_PassingTDs)+
         ...
         pc3 = (weighted_RushingYards*0.96620788)+(weighted_RushingTDs*0.93854323),
         ovr = -pc1+pc2+pc3) %>%
  select(Rank2023,PositionRank2023,Team2023,FantasyPosition2023,Player2023,FantasyPointsPPR2023,pc1,pc2,pc3,ovr) %>%
  arrange(desc(ovr)) %>%
  mutate(ModelRank = row_number(), .before = Rank2023) %>%
  tibble()
...
```{r, echo = FALSE, eval = FALSE, include = FALSE, warning=FALSE }
