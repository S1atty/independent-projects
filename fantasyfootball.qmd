...
#                         Player=gsub("\\+","",Player))  
fantasy2023 <- data.frame(read.xlsx("Fantasy Football Data/archive/fantasy2023.xlsx"))
...
fantasy2023 %<>% mutate(across(c(1,3,5:26),as.numeric))
...
fantasy2023 %>%
  select(FantasyPosition,Player,PlayerID,FantasyPointsPPR) %>%
  filter(FantasyPosition!="FB") %>%
  group_by(FantasyPosition) %>%
  slice_max(FantasyPointsPPR,n=10) %>%
  arrange(desc(FantasyPosition)) %>%
  tibble() %>%
  print(n=40)
